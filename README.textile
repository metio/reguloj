...
final RuleEngine chained = RuleEngines.chained();

// All rules will be evaluated, but only a maximum number of 5 times.
final RuleEngine limited = RuleEngines.limited(5);

// Evaluates all rules, stops after the first one that fires.
final RuleEngine firstWins = RuleEngines.firstWins();
...
p. This project supplies a simple implementation of the @Context@ interface called @BaseContext@. Subclass @BaseContext@ in case your rules need extra information.
